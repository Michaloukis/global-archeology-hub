...
export default function JournalScreen({ siteId, onBack }) {
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.subtitle}>{siteId ? `Site: ${siteId}` : 'Field journal'}</Text>
      <View style={styles.card}>
        <Text style={styles.cardText}>
          Full journal terminal is available on the web app.
        </Text>
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 14, fontWeight: '600', color: colors.ink },
