...
function toGrayscale(data, _w, _h) {
  const out = [];
...
function blur5x5(gray, width, height) {
  const out = gray.slice();
  const w = width;
  const h = height;
  for (let y = 2; y < h - 2; y++) {
...
function binarize(gray, threshold, darkIsForeground) {
...
function medianFilter(arr, halfWindow) {
  const out = [];
  const n = arr.length;
  const buf = [];
  for (let i = 0; i < n; i++) {
 ...
 */
export function extractProfileFromImage(image, options = {}) {
  ...
  function detectBackgroundColor(gray, w, h) {
    const borderPixels = [];
  ...
  function fillFromEdges(fg, w, h) {
    ...
    while (queue.length > 0) {
      ...
      if (idx === undefined || visited.has(idx) || fg[idx]) continue; // Skip if visited or is foreground
      visited.add(idx);
  ...
  function extractOuterContour(fg, w, h) {
