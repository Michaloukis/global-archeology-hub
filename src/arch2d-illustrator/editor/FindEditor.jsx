...
import { extractProfileFromImage } from '../utils/photoToProfile';
...
export function FindEditor({ initialSpec, onReset, onSave }) {
  ...
  const updateProfilePoint = useCallback((index, point) => {
    setSpec((s) => {
  ...
  const addProfilePoint = useCallback((afterIndex) => {
    setSpec((s) => {
      const profile = [...s.profile];
  ...
  const removeProfilePoint = useCallback((index) => {
    setSpec((s) => {
  ...
  const updateDecorationBand = useCallback((index, band) => {
    setSpec((s) => {
  ...
  const removeDecorationBand = useCallback((index) => {
    setSpec((s) => ({
      ...s,
  ...
  const updateHandle = useCallback((index, patch) => {
    setSpec((s) => {
  ...
  const removeHandle = useCallback((index) => {
    setSpec((s) => {
      const list = s.handles ?? (s.handle ? [s.handle] : []);
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] md:text-xs text-stone-600">
            <label className="flex items-center gap-1">
              ...
              <select
                ...
                onChange={(e) => setExtractOptions((o) => ({ ...o, blur: Number(e.target.value) }))}
            ...
            <label className="flex items-center gap-1" title="Fill holes and remove specks (helps reflective or noisy photos)">
              ...
              <select
                ...
                onChange={(e) => setExtractOptions((o) => ({ ...o, cleanup: Number(e.target.value) }))}
                className="rounded border border-stone-300 px-1 py-0.5 text-xs"
              >
                <option value={0}>Off</option>
        ...
        <div className="flex flex-wrap items-center gap-1.5 md:gap-3">
          ...
          <label className="flex items-center gap-1">
            ...
            <select
              ...
              onChange={(e) => update('baseDetail', e.target.value)}
              className="rounded border border-stone-300 px-1 py-0.5 text-xs"
            >
            ...
            <ul className="space-y-0.5 md:space-y-1">
              {spec.decorationBands.map((band, i) => (
                <li key={i} className="flex flex-wrap items-center gap-1 py-1 rounded bg-stone-50 border border-stone-200 px-1.5">
                  <select
                    value={band.type}
                    onChange={(e) => updateDecorationBand(i, { type: e.target.value })}
            ...
            <>
              {handleList.map((h, i) => (
                <div key={i} className="flex items-center gap-1.5 py-0.5 flex-wrap">
                  ...
                  <select
                    value={h.side ?? 'left'}
                    onChange={(e) => updateHandle(i, { side: e.target.value })}
