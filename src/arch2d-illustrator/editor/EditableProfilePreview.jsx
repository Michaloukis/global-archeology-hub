...
function profileToSvg(p, widthScale, heightScale) {
  return {
...
export function EditableProfilePreview({
  ...
  const handleProfilePointerDown = useCallback((e, index) => {
    e.preventDefault();
    e.target.setPointerCapture(e.pointerId);
    setDragging({ type: 'profile', index });
  }, []);

  const handleHandlePointerDown = useCallback((e, handleIndex, end) => {
    e.preventDefault();
    e.target.setPointerCapture(e.pointerId);
    setDragging({ type: 'handle', handleIndex, end });
  }, []);

  const handleCenterlinePointerDown = useCallback((e) => {
    e.preventDefault();
    e.target.setPointerCapture(e.pointerId);
